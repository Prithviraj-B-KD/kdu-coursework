...
public class Market {
    ...
    public static synchronized Market getMarket_instance (){
    ...
    private Market() {
            ...
            if (tradersCsvParser != null) {
                for (CSVRecord csvRecord : tradersCsvParser) {
                    ...
                    traders.put(walletAddress,new  Trader(firstName, lastName, phoneNumber, walletAddress));
                    traderCount++;
                }
            }
            System.out.println(this.coins.values());
            try {
    ...
    public void getTopNCoins (int n){
        ...
        List<Coin> topNcoins= null;
            topNcoins = coins.values().stream().sorted(Comparator.comparing(Coin::getRank).thenComparing(Coin::getName)).limit(n).toList();
    ...
    public Optional<Trader> findTraderByName(String name){
        return this.traders.values().stream().filter(trader -> trader.getFirstName().equalsIgnoreCase(name))
                .findFirst();

    }
    public Optional<Trader> findTraderByWalletAddress(String walletAddress){
        return Optional.ofNullable(traders.get(walletAddress));
    }
    public Optional<Coin> findCoinByName(String name){
        return coins.values().stream().filter(coin -> (coin.getName().equalsIgnoreCase(name)) || (coin.getSymbol().equalsIgnoreCase(name)))
                .findFirst();
    }

    public void getTopTradersByProfit(int n){
        List<Trader> topTraders= null;
        topTraders =  traders.values().stream().sorted(Comparator.comparing(Trader::getTotalProfitLoss).reversed()).limit(n).toList();
    ...
    public void getTopTradersByLoss(int n){
        List<Trader> topTraders= null;
        topTraders =  traders.values().stream().sorted(Comparator.comparing(Trader::getTotalProfitLoss)).limit(n).toList();
